...
class CausalCurriculumController:
    ...
    def get_weights(self, epoch):
        decay_factor = np.exp(-epoch / self.tau)
        weights = {
            'stage1': 0.5 + self.alphas['stage1'] * decay_factor,
            'stage2': 0.5 + self.alphas['stage2'] * decay_factor,
            'stage3': 0.5 + self.alphas['stage3'] * decay_factor,
            'stage4': 0.5 + self.alphas['stage4'] * decay_factor
        }
        return weights
...
if __name__ == "__main__":
    print(f"\nStarting physics-causal curriculum learning training...")
    ...
    print(f"Model built: CASCADE ({best_config['fusion_type'].upper()}) | Total params: {count_parameters(model)}")

    criterion = nn.MSELoss()
    optimizer = optim.AdamW(model.parameters(), lr=best_config['lr'], weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=MAX_EPOCHS, eta_min=1e-5)

    curriculum_controller = CausalCurriculumController(tau=100.0)

    history = []

    print("Starting curriculum-weighted training...")
    for epoch in range(1, MAX_EPOCHS + 1):
        model.train()
        train_tracker = MetricTracker()

        current_weights = curriculum_controller.get_weights(epoch)
        ...
        for x_dict_batch, y_true_dict in train_loader:
            ...
            total_loss = (current_weights['stage1'] * loss_s1 +
                          current_weights['stage2'] * loss_s2 +
                          current_weights['stage3'] * loss_s3 +
                          current_weights['stage4'] * loss_s4)
        ...
        with torch.no_grad():
            for x_dict_batch, y_true_dict in val_loader:
                ...
                v_total_loss = (current_weights['stage1'] * v_loss_s1 +
                                current_weights['stage2'] * v_loss_s2 +
                                current_weights['stage3'] * v_loss_s3 +
                                current_weights['stage4'] * v_loss_s4)
        ...
        epoch_metrics = {
            ...
            "weight_stage1": current_weights['stage1'],
            "weight_stage2": current_weights['stage2'],
            "weight_stage3": current_weights['stage3'],
            "weight_stage4": current_weights['stage4']
        }
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch:03d} | Val Avg R2: {val_res['total_r2']:.4f}")
            print(f"          > Annealed weights: S1({current_weights['stage1']:.2f}) S2({current_weights['stage2']:.2f}) S3({current_weights['stage3']:.2f}) S4({current_weights['stage4']:.2f})")

    csv_filename = "training_history_curriculum.csv"
    history_df = pd.DataFrame(history)
    history_df.to_csv(csv_filename, index=False)
    print(f"Training complete. History saved to {csv_filename}")
    ...
    with torch.no_grad():
        for x_dict_batch, _ in infer_loader:
            ...
            out_csv = "last_50_predictions_curriculum_with_raw.csv"
